...
async with build_http_scraper(
    ...
    proxy_maximum_requests=100,
    proxy_maximum_bytes=25_000_000,
) as scraper:
...
`warning`. Diagnose a request by `collection_id`, then provider, endpoint,
lease, attempt, classification, byte totals, and elapsed time.
