...
class _BrowserDispatchState:
    origins_validated: bool = False
    dispatched: bool = False


@dataclass(frozen=True, slots=True)
class _BrowserDispatchResult:
    status: int
...
class _BrowserSessionOwner:
    """Lazily own one backend session with an optional application job."""

    def __init__(
        self,
        backend: BrowserBackend,
        job: BrowserJobContext | None = None,
    ) -> None:
        self._backend = backend
        self._job = job
        self._context: AbstractAsyncContextManager[BrowserSession] | None = None
...
async def _dispatch(
    ...
    *,
    origins: _BrowserOriginPolicy,
    sessions: _BrowserSessionOwner,
    state: _BrowserDispatchState,
) -> _BrowserDispatchResult:
    endpoint = (
    ...
    origins.validate(endpoint)
    origins.validate(page_url)
    state.origins_validated = True
    ...
    if request.evaluation is not None:
        ...
        origins.validate_response(result.final_url)
        return _BrowserDispatchResult(
            status=200,
            headers={"content-type": "application/json"},
            content=content,
            final_url=result.final_url,
        )
    if method == "GET" and not request.headers and request.json_body is None:
    ...
    origins.validate_response(response.final_url)
    return _BrowserDispatchResult(
...
class BorrowedBrowserTransport:
    ...
    def __init__(
        ...
        *,
        ...
        self._sessions = _BrowserSessionOwner(backend, job)
        ...
        async with self._lock:
            ...
            try:
                dispatched = await _dispatch(
                    request,
                    origins=self._origin_policy,
                    sessions=self._sessions,
                    state=state,
                )
            except asyncio.CancelledError as error:
                error.accounting = self._accounting(  # type: ignore[attr-defined]
                    request, physical_requests=1 if state.dispatched else 0
                )
                raise
            except BrowserOriginDenied:
                if not state.origins_validated:
                    raise
                raise TransportFailure(
                    "browser transport failed: BrowserOriginDenied",
            ...
            accounting = self._accounting(
                request, received_bytes=len(dispatched.content)
            )
            return enforce_response_body_limit(
                TransportResponse(
                    ...
                    final_url=dispatched.final_url,
...
class CamoufoxProxyBrowserTransport:
    ...
    def __init__(
        ...
        *,
        ...
        self._origin_policy = _BrowserOriginPolicy(allowed_origins)
        self._sessions = _BrowserSessionOwner(self._backend)
    ...
    async def request(self, request: TransportRequest) -> TransportResponse:
        ...
        async with self._request_lock:
            ...
            try:
                ...
                if not state.origins_validated:
                    raise
                failure = ("backend", "browser transport failed: BrowserOriginDenied")
            except BrowserUnavailable:
            ...
            accounting = self._delta(before, retained_bytes=len(dispatched.content))
