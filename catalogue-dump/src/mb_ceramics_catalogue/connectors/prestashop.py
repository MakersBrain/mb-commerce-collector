...
from .page import gtin as jsonld_gtin
from .page import images as jsonld_images
from .page import offer as jsonld_offer


class PrestaShopTransport(Protocol):
...
class PrestaShopConnector(CommerceConnector):
    ...
    async def collect(
        ...
        try:
            ...
            yield self._budget_failure("discovery", 0, error)
            return
        ...
        if resume is not None and resume[0] not in keys:
            raise ValueError("PrestaShop checkpoint partition is not requested")
        ...
        for index, (partition, offset, url) in enumerate(selected):
            ...
            try:
                ...
                yield self._budget_failure(partition, sequence, error, offset=offset)
                return
            except (_BoundReached, httpx.HTTPError, RuntimeError) as error:
                yield self._failure_page(
                    partition,
                    sequence,
    ...
    async def _discover(self, request: CollectionRequest) -> list[tuple[str, list[str]]]:
        ...
        if not sitemap_roots and self.options.use_advertised_sitemaps:
            ...
            sitemap_roots = list(await self.transport.advertised_sitemaps(request.base_url))
        ...
        if self.options.sitemaps and not self.options.category_urls:
            raise RuntimeError("configured sitemaps yielded no product URLs")
    ...
    async def _fetch(
        ...
        *,
        ...
        if request.request_budget is not None and self._requests >= request.request_budget:
            raise _BoundReached("request budget")
        if not await self.transport.allowed(url):
            raise RuntimeError("robots policy denied URL")
        self._budget.require(priority, url, browser=rendered)
        self._requests += 1
        return await self.transport.document(url, rendered=rendered, accept=accept)
    ...
    async def _combinations(
        ...
        priority = self._budget.required_detail_priority(
        ...
        for target in wanted:
            try:
                ...
                if priority == RequestPriority.DATASET_REQUIRED:
                    raise
                notices.append(
                    Diagnostic(
                        code=DiagnosticCode.OPTIONAL_ENRICHMENT_SKIPPED,
                        severity=DiagnosticSeverity.WARNING,
                        message="PrestaShop combination fetch deferred by request budget",
                        retryable=True,
                        affects_completeness=False,
                ...
                break
            except (_BoundReached, httpx.HTTPError, RuntimeError, json.JSONDecodeError) as error:
                notices.append(
                    Diagnostic(
                        code=DiagnosticCode.OPTIONAL_ENRICHMENT_SKIPPED,
                        severity=DiagnosticSeverity.WARNING,
                        message=f"PrestaShop combination fetch skipped: {type(error).__name__}",
                        retryable=isinstance(error, (httpx.HTTPError, RuntimeError)),
                        affects_completeness=False,
                        url=_safe_url(target),
                    )
                )
    ...
    def _budget_failure(
        ...
        *,
        ...
        return EntityPage(
            page_id=f"{partition}:{sequence}:budget",
            partition_key=partition,
            sequence=sequence,
            items=(),
            resume_after={
            ...
            discovered=0,
            diagnostics=(budget_diagnostic(error.priority, _safe_url(error.url)),),
        )
