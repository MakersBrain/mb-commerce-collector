...
class WixTransport(Protocol):
    """Backend-neutral access required by the public Wix storefront connector."""
...
class WixOptions(BaseModel):
    """Connector-owned projection of the legacy Wix source configuration."""
...
class WixConnector(CommerceConnector):
    ...
    async def collect(
        ...
        try:
            ...
            yield self._failure_page(sequence, start, budget_diagnostic(error.priority, error.url))
            return
        ...
        if not urls:
            ...
            return
        if start > len(urls):
            raise ValueError("Wix checkpoint index is beyond discovered products")
        ...
        for index in range(start, stop):
            ...
            try:
                ...
                yield self._failure_page(
                    sequence, index, budget_diagnostic(error.priority, error.url)
                )
                return
            except (httpx.HTTPError, RuntimeError) as error:
                yield self._entity_failure(sequence, index, url, error)
                return
            snapshot = self._normalize(document, url, request.source_id, self._clock())
            if snapshot is None and self.options.render is None:
                try:
                    ...
                    rendered = ""
                snapshot = self._normalize(rendered, url, request.source_id, self._clock())
            if snapshot is None:
                yield self._failure_page(
                    sequence,
                    index,
            ...
            yield EntityPage(
                ...
                resume_after=(
                    ...
                    else {"index": next_index, "sequence": sequence + 1}
                ),
                terminal=terminal,
        ...
        # A configured safety bound is also an incomplete enumeration.
        if stop < len(urls):
            yield self._failure_page(
                sequence,
                stop,
    ...
    async def _walk_sitemaps(self, queue: list[str]) -> list[str]:
        ...
        while queue:
            ...
            if len(seen) >= self.options.sitemap_limit:
                raise RuntimeError("Wix sitemap traversal limit reached")
            seen.add(url)
    ...
    def _normalize(
        ...
        title = _clean(product.get("name")) or _clean(jsonld_product.get("name")) or _meta(
            document, "og:title"
        )
        ...
        brand = _clean(product.get("brand")) or self.options.brand
        evidence = Evidence(
            method="html",
        ...
        variants = tuple(
            ...
            if (variant := self._variant(raw, product_id, canonical_url, currency, evidence))
            is not None
        )
    ...
    def _variant(
        ...
        return CommerceVariant(
            ...
            published_attributes={
                "price_text": _clean(raw.get("formattedPrice")) or None,
            },
    ...
    def _validate_request(
        ...
        if checkpoint is not None and (
            ...
            raise ValueError("Wix checkpoint does not match this collection")
    ...
    def _resume(checkpoint: ConnectorCheckpoint | None) -> tuple[int, int]:
        ...
        if not isinstance(cursor, dict):
            raise ValueError("Wix checkpoint cursor must be an object")
        ...
        if (
            ...
            raise ValueError("Wix checkpoint cursor is invalid")
