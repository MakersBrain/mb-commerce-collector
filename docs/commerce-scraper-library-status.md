...
- [x] Current implementation batch passed the verification gates recorded
  ...
  - Owned HTTPX clients now use a strict 32-entry, concurrency-safe LRU.
    ...
    closing all retained owned clients. Lifecycle regressions passed alongside
    the complete scraper gate (349 tests), full catalogue gate, and installed
  ...
  - Generic and vendor page connectors now share one independently housed page
    engine and one nested options model. The generic conversion layer and flat
    specialized duplicate are gone, and catalogue runtime plans serialize the
...
- [~] Run recorded-response replay and ceramics projection comparisons.
  ...
  - The Art4Fun Starweb recording passes with 1,713 ordered rows and 2,288
...
- Starweb discovery counters describe different layers: legacy reports unique
  product-page URLs, while the neutral page engine reports parsed Product
  entities and can observe several on one URL. The recorded gate pins both
  totals while requiring exact ordered rows, requests, and terminal behavior.
